...
export default function OnboardingLoader({ 
  ...
  const progressWidth = Math.min(currentProgress / 100 * 31, 31);

  return (
    <div className="fixed inset-0 w-full h-full flex flex-col items-center justify-center z-[9999] overflow-hidden bg-[#0F172A]">
      {/* MODEL 로고 */}
      <motion.div 
      ...
      >
        <div className="relative w-[203px] h-[57px] rounded-[28.5px] flex items-center justify-center font-bold text-[28px] text-slate-100 tracking-[2px]"
             style={{
  ...
  return (
    <div className="fixed inset-0 w-full h-full flex flex-col items-center justify-center z-[9999] overflow-hidden bg-[#0F172A] md:max-w-[375px] md:max-h-[676px] md:mx-auto md:rounded-[20px] md:shadow-[0_20px_40px_rgba(0,0,0,0.3)]">
      ...
      >
        ...
        {/* 활성 프로그레스 바 */}
        <motion.div 
          className="absolute top-0 left-0 h-full bg-[#ED172B] rounded transition-all duration-300 ease-out"
          style={{
            width: `${progressWidth}%`,
            boxShadow: '0 0 10px rgba(237, 23, 43, 0.5), 0 0 20px rgba(237, 23, 43, 0.3)'
          }}
          initial={{ width: 0 }}
